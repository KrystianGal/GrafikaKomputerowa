...
class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
    ...
    public void Korkociag(GL2 gl2,int radius, int numberOfTurns){
        ...
        int numVertices = numberOfTurns*11;
        ...
        for (int i = 1; i <= numVertices; i++) {
            ...
            gl2.glColor3f(x/0.0f,(x+z)*x/0.5f,z/0.5f);
            x = (float) (radius * Math.cos(angle));
            z = (float) (radius * Math.sin(angle));
            gl2.glVertex3f(x, height, z);
            height+=numberOfTurns/400f;
        }
        gl2.glEnd();

        gl2.glTranslated(0,height,-4.5);
        glu.gluCylinder(glu.gluNewQuadric(),1.0,1.0,9,100,50);
    }
    ...
    public void PiramidaZad(GL2 gl2,double size){
